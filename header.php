 ...
 */
$bannerOpts     = get_field( 'banner', 'option' ) ?: array();
$home_id        = get_option( 'page_on_front' );
$quizUrl        = $home_id ? get_field( 'quizz_url', $home_id ) : '';
$quizLabel      = $home_id ? get_field( 'take_a_quizz_label', $home_id ) : '';
$lookAroundOpts = get_field( 'menu_look_around', 'option' ) ?: array();
$socialMediaOpts = get_field( 'social_media', 'option' ) ?: array();
?>
<!doctype html>
...
<body <?php body_class(); ?>>
	...
	<div id="page" class="site">
		...
		<header id="masthead" class="site-header">
			<?php if ( $bannerOpts ) : ?>
				<div class="header-banner" style="background-color: <?php echo esc_attr( $bannerOpts['banner_background_color'] ?? '' ); ?>;">
					<div class="container align-items-center">
						<a id="colophonBtn" href="#advertiser-disclosure">ADVERTISER AND EDITORIAL DISCLOSURE</a>
						<?php if ( ! empty( $bannerOpts['banner_image'] ) ) : ?>
							<div class="header-banner__img">
								<img src="<?php echo esc_url( $bannerOpts['banner_image']['url'] ); ?>" alt="<?php echo esc_attr( $bannerOpts['banner_image']['alt'] ); ?>">
							</div>
						<?php endif; ?>
						<div class="header-banner__content">
							<?php if ( ! empty( $bannerOpts['banner_title'] ) ) : ?>
								<h2 class="header-banner__content__title"><?php echo esc_html( $bannerOpts['banner_title'] ); ?></h2>
							<?php endif; ?>
							<?php if ( ! empty( $bannerOpts['banner_description'] ) ) : ?>
								<div class="header-banner__content__description">
									<?php echo wp_kses_post( $bannerOpts['banner_description'] ); ?>
								</div>
							<?php endif; ?>
						</div>
						<?php if ( ! empty( $bannerOpts['cta_link'] ) ) : ?>
							<div class="header-banner__cta" target="_blank" rel="noopener noreferrer">
								<a href="<?php echo esc_url( $bannerOpts['cta_link'] ); ?>" target="_blank" rel="noopener noreferrer" class="btn btn-header">
									<?php echo esc_html( $bannerOpts['cta_text'] ?? '' ); ?>
								</a>
							</div>
						<?php endif; ?>
					</div>
    ...
    <div class="look-around__quiz-link">
        <a class="take-quizz" href="<?php echo esc_url( $quizUrl ); ?>" target="_blank" rel="noopener noreferrer">
            <?php echo esc_html( $quizLabel ); ?>
        </a>
    </div>
<?php endif; ?>
					<div class="look-around__big-menu">
						<?php if ( $lookAroundOpts ) : ?>
							<div class="label">
								<span><?php echo esc_html( $lookAroundOpts['look_around_text'] ?? '' ); ?></span>
							...
							<div class="look-around__menu">
								<div class="about-ofp">
									<div class="content">
										<?php if ( ! empty( $lookAroundOpts['image'] ) ) : ?>
											<img class="kam-img" src="<?php echo esc_url( $lookAroundOpts['image']['url'] ); ?>" alt="<?php echo esc_attr( $lookAroundOpts['image']['alt'] ); ?>">
										<?php endif; ?>
										<?php the_custom_logo(); ?>
										<h2 class="title">
											<?php echo esc_html( $lookAroundOpts['title'] ?? '' ); ?>
										</h2>
										<div class="description">
											<?php echo wp_kses_post( $lookAroundOpts['description'] ?? '' ); ?>
										</div>
									</div>
									<div class="social-media">
										<div class="instagram">
											<a href="<?php echo esc_url( $socialMediaOpts['instagram_url'] ?? '' ); ?>" target="_blank" rel="noopener noreferrer">
										...
										<div class="facebook">
											<a href="<?php echo esc_url( $socialMediaOpts['facebook_url'] ?? '' ); ?>" target="_blank" rel="noopener noreferrer">
										...
										<div class="pinterest">
											<a href="<?php echo esc_url( $socialMediaOpts['pinterest_url'] ?? '' ); ?>" target="_blank" rel="noopener noreferrer">
								...
								<div class="center">
									<?php foreach ( $lookAroundOpts['center_words'] ?? array() as $word ) : ?>
										<span><?php echo esc_html( $word['word'] ); ?></span>
									<?php endforeach; ?>
								</div>
