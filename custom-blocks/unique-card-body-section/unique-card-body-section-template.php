...
$blockID = 'unique-card-body-section-' . uniqid();
?>

<style>
  .unique-card__body-section__card-title {
    color: <?= $title_color ?>;
  }
  .unique-card__body-section__card-new-offer {
    color: <?= $new_offer_color ?>;
  }
  .unique-card__body-section__card-old-offer {
    color: <?= $old_offer_color ?>;
  }
  .unique-card__body-section__card-current-offer {
    color: <?= $current_offer_color ?>;
  }
  .unique-card__body-section__card-cta,
  .unique-card__body-section__card-cta:visited {
    color: <?= $learn_more_color ?>;
  }
  .unique-card__body-section__card-cta:hover,
  .unique-card__body-section__card-cta:hover:visited {
    color: <?= $learn_more_hover_color ?>;
  }
  .unique-card__body-section__card-fees,
  .unique-card__body-section__card-fees:visited {
    color: <?= $rates_and_fees_color ?>;
  }
  .unique-card__body-section__card-fees:hover,
  .unique-card__body-section__card-fees:hover:visited {
...
<div id="<?= $blockID ?>" class="unique-card__body-section">
    <div class="unique-card__body-section__card">
      <?php if ($image || $rotating_image) : ?>
        <div class="unique-card__body-section__rotating-img">
      ...
      <div class="unique-card__body-section__card-content">
        <?php if (!empty($card_name)) : ?>
          <h2 class="unique-card__body-section__card-title">
            <?= $card_name ?>
          </h2>
        <?php endif; ?>
        
        <?php if (!empty($new_offer) || !empty($old_offer)) : ?>
          <div class="unique-card__body-section__offers-container">
            <?php if (!empty($new_offer)) : ?>
              <h3 class="unique-card__body-section__card-new-offer">
                <?= $new_offer ?>
              </h3>
            <?php endif; ?>
            
            <?php if (!empty($old_offer)) : ?>
              <h4 class="unique-card__body-section__card-old-offer">
                <?= $old_offer ?>
              </h4>
            <?php endif; ?>
          </div>
        ...
        <?php if (!empty($current_offer)) : ?>
          <div class="unique-card__body-section__card-current-offer">
            <?= $current_offer ?>
        ...
        <?php if (!empty($learn_more_link)) : ?>
          <a class="btn unique-card__body-section__card-cta" href="<?= esc_url($learn_more_link) ?>" target="_blank" rel="noopener noreferrer">
      ...
      <?php if (!empty($card_image['url'])) : ?>
        <div class="unique-card__body-section__card-img">
          <img src="<?= esc_url($card_image['url']) ?>" alt="<?= $card_name ?>">
          
          <?php if (!empty($rates_and_fees)) : ?>
            <a class="btn unique-card__body-section__card-fees" href="<?= esc_url($rates_and_fees) ?>" target="_blank" rel="noopener noreferrer">
              RATES AND FEES
            </a>
          <?php endif; ?>
        </div>
